...
- (id)initWithMessage:(NSString *)message ImageUrl :(NSString *)ImageUrl  buttonTitles:(NSArray *)btnArr
{

    CGRect rect = CGRectMake(0, 0, CGRectGetWidth([UIScreen mainScreen].bounds), CGRectGetHeight([UIScreen mainScreen].bounds)+20);
...
{ 
    ...
    CGFloat image_width = VIEW_With*140.00/320;

    rect = CGRectMake((alertSize.width-image_width)/2, 20.00*VIEW_With/320, image_width, image_width);
    UIImageView *ttImageView = [[UIImageView alloc]init];
    if (topImageUrl != nil) {
        [ttImageView sd_setImageWithURL:[NSURL URLWithString:topImageUrl]];
    }
...
- (void)addAlertLabels
{
    CGRect rect;
         ...
         CGFloat image_Height = VIEW_With*140.00/320;


    CGFloat lab_Y = image_Height+20.00*VIEW_With/320;
 
    //19开始，宽220 
    CGFloat width = alertSize.width-60;
    rect = CGRectMake((CGRectGetWidth(_bgImageView.frame)-width)/2, lab_Y , width, CGRectGetHeight(_bgImageView.frame)-lab_Y - 55);
    self.contextLabel = [[UITextView alloc]initWithFrame:rect];
    self.contextLabel.editable = NO;
...
//    self.contextLabel.backgroundColor = [UIColor redColor];
...
//        {
        ...
        if (btnTitleArr.count == 1) {
            btn.frame = CGRectMake(40, CGRectGetHeight(_bgImageView.frame)-50, CGRectGetWidth(_bgImageView.frame)-80, 38);
        }else{
